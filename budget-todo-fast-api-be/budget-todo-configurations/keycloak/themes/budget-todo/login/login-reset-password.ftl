...
<@layout.registrationLayout displayInfo=true; section>
    ...
    <#elseif section = "form">
        <form id="kc-reset-password-form" class="form reset-password ${properties.kcFormClass!}" action="${url.loginAction}" method="post">
            ...
            <div class="${properties.kcFormGroupClass!} row forgot-btn">
                <div id="kc-form-options" class="${properties.kcFormOptionsClass!} col-xs-5">
                    <div class="${properties.kcFormOptionsWrapperClass!}">
                        <span>
                            <a class="btn btn-default btn-flat btn-block btn-back" href="${url.loginUrl}"><i class="fa fa-caret-left display"></i>&nbsp;&nbsp;${msg("backToLogin")}</a>
                        </span>
                    </div>
                </div>

                <div id="kc-form-buttons" class="${properties.kcFormButtonsClass!} col-xs-7">
                    <input class="btn btn-primary btn-flat btn-block ${properties.kcButtonClass!} ${properties.kcButtonPrimaryClass!} ${properties.kcButtonLargeClass!}" style="border-radius: 40px;" type="submit" value="${msg("doSubmit")}"/>
                </div>
            </div>
        </form>
    <#elseif section = "info" >
